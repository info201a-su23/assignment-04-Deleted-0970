...
* Average jail rate of different racial groups (LatinX & White)

* How has the jail rate changed in the past decade in Washington state for the
LatinX community? 
...
* Which year had the greatest difference in jail rates between the Latinx and
White racial groups?
...
# change to include = FALSE when turning in
source("summary.R")
...
***
Given that the LatinX community and the White community has the largest 
population within the United States we will examine the difference in 
incarceration rates between these two groups.
...
the United States data set, we can see a noticeable difference between 
these two communities with the LatinX community experiencing an average of 
`r avg_latinx_jail` people jailed per 100,000 while the White community 
experiences an average of `r avg_white_jail` people jailed per 100,000. 
